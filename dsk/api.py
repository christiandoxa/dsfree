...
class DeepSeekAPI:
    ...
    def __init__(self, auth_token: str):
        ...
        try:
            ...
            print(f"\033[93mWarning: Could not parse cookies from {cookies_path}: {e}\033[0m", file=sys.stderr)
            self.cookies = {}
        ...
        while retry_count < max_retries:
            try:
                ...
                if isinstance(response_data, dict) and response_data.get('code') not in (None, 0):
                    message = response_data.get('msg') or response_data.get('message') or str(response_data)
                    if response_data.get('code') == 40003:
    ...
    def _parse_chunk(self, chunk: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            if chunk.startswith(b'data: '):
                ...
                if path in ('response/content', 'response/thinking_content') and data.get('o') == 'APPEND':
                    stream_type = 'thinking' if path == 'response/thinking_content' else 'text'
                ...
                # Some patch continuations omit the path and only carry {"v": "..."}.
                if set(data.keys()) == {'v'} and isinstance(data.get('v'), str):
                    return {
                        'content': data.get('v', ''),
                        'type': self._last_stream_type or 'text',
                        'finish_reason': None
                    }
